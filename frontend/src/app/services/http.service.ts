...
export class HttpService {
  ...
  subscribeUser(autUser: Autreguser, service: Service): Observable<User> {
    ...
    return this.http.post<User>('http://localhost:8081/api/user/join?service_id=' + service.id, autUser);
  }

  unsubscribeUser(autUser: Autreguser, service: Service): Observable<User> {
    console.log(service.id);
    return this.http.post<User>('http://localhost:8081/api/user/refuse?service_id=' + service.id, autUser);
  }

  deleteService(serviceId: number): Observable<void> {
    return this.http.delete<void>('http://localhost:8081/api/service/delete/' + serviceId);
  }
  ...
  public getLoggedUser(login: string): Observable<User> {
    return this.http.post<User>('http://localhost:8081/api/user/login', login);
  }
  ...
  saveService(service: Service): Observable<Service> {
    return this.http.post<Service>('http://localhost:8081/api/service', service);
  }
  ...
  saveWallet(wallet: Wallet): Observable<Wallet> {
    return this.http.post<Wallet>('http://localhost:8081/api/wallet', wallet);
  }
